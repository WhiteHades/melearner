...
- `src-tauri/src/native_player/surface.rs` owns the current same-process window-handle video surface. Its diagnostic backend label is `window-handle:*` and `surfaceRenderApi` is `false` until a verified render-API backend replaces it.
