...
- Linux uses a GTK overlay inside the existing Tauri/WebKit window. The WebView remains the UI layer, while libmpv renders into a GTK `GLArea` positioned over the measured video placeholder. If this path cannot attach, playback must fail clearly instead of opening a second compositor-visible app window. Windows and macOS must follow the same one-window product rule when their platform-native render hosts are completed.
