...
- Settings contains the current root and root actions, Light/Dark/Cozy appearance, reduced-motion/high-contrast status inherited from the OS, and build/runtime information. Appearance applies immediately and persists as a typed value. Its independent Settings revision starts at `1` for the fresh Light default and advances without changing the Library revision. There is no marker-file setting.
