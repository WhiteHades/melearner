...
if "$package_script" \
    ...
    --output "$work_dir/melearner-0.1.9-linux-x86_64.tar.zst" \
...
printf '%s\n' \
  ...
  'project(melearner VERSION 0.1.9 LANGUAGES CXX)' \
  >"$valid_source/CMakeLists.txt"
printf '%s\n' 'CMAKE_PROJECT_VERSION:STATIC=0.1.9' >"$valid_build/CMakeCache.txt"
...
if grep -Fq "configured CMake build version must be 0.1.9" "$work_dir/valid-stage.log"; then
  echo "stager rejected a valid 0.1.9 CMake cache" >&2
  exit 1
fi
