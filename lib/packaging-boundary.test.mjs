...
test("ci workflow covers supported Linux native-player checks", () => {
  ...
  assert.equal(workflow.includes("npm run test:parity"), true)
...
test("desktop app prevents duplicate heavy library bootstraps", () => {
  ...
  assert.equal(cargoToml.includes('tauri-plugin-single-instance = "=2.4.2"'), true)
  assert.equal(tauriEntrypoint.includes("tauri_plugin_single_instance::init"), true)
  assert.equal(tauriEntrypoint.includes('write_startup_log("single-instance.activate")'), true)
  assert.equal(tauriEntrypoint.includes('app.get_webview_window("main")'), true)
  assert.equal(tauriEntrypoint.includes("window.set_focus()"), true)
})
...
test("windows msi workflow uploads a signed artifact without publishing a release", () => {
  const workflow = readFileSync(join(repoRoot, ".github/workflows/windows-msi.yml"), "utf8")
  const releaseWorkflow = readFileSync(join(repoRoot, ".github/workflows/release.yml"), "utf8")
  const cargoToml = readFileSync(join(repoRoot, "src-tauri/Cargo.toml"), "utf8")
  const installDoc = readFileSync(join(repoRoot, "docs/install.md"), "utf8")
  const signingScript = readFileSync(join(repoRoot, "scripts/sign-windows-msi.ps1"), "utf8")
  ...
  assert.equal(workflow.includes("prepare windows signing certificate"), true)
  assert.equal(workflow.includes("shell: bash"), true)
  assert.equal(workflow.includes("WINDOWS_CODE_SIGNING_PFX_BASE64"), true)
  assert.equal(workflow.includes("WINDOWS_CODE_SIGNING_PFX_PASSWORD"), true)
  assert.equal(workflow.includes("WINDOWS_CODE_SIGNING_TIMESTAMP_URL"), true)
  assert.equal(workflow.includes("WINDOWS_SIGNING_MODE=production"), true)
  assert.equal(workflow.includes("WINDOWS_SIGNING_MODE=test"), true)
  assert.equal(workflow.includes("WINDOWS_SIGNING_ALLOW_UNTRUSTED_ROOT=true"), true)
  assert.equal(workflow.includes("WINDOWS_SIGNING_PFX"), true)
  assert.equal(workflow.includes("WINDOWS_SIGNING_PFX_PASSWORD"), true)
  assert.equal(workflow.includes("melearner-windows-msi-signing.txt"), true)
  ...
  assert.equal(workflow.includes("sign msi"), true)
  assert.equal(workflow.includes("timeout-minutes: 3"), true)
  assert.equal(workflow.includes("scripts\\sign-windows-msi.ps1"), true)
  ...
  assert.equal(signingScript.includes('"/tr", $TimestampUrl, "/td", "SHA256"'), true)
  assert.equal(signingScript.includes('@("verify", "/pa", "/v", $Path)'), true)
  assert.equal(signingScript.includes("[switch]$AllowUntrustedRoot"), true)
  ...
  assert.equal(signingScript.includes("signature chain is not trusted"), true)
  ...
  assert.equal(installDoc.includes("WINDOWS_CODE_SIGNING_PFX_BASE64"), true)
  assert.equal(installDoc.includes("WINDOWS_CODE_SIGNING_PFX_PASSWORD"), true)
  assert.equal(installDoc.includes("WINDOWS_CODE_SIGNING_TIMESTAMP_URL"), true)
  assert.equal(installDoc.includes("mode=production"), true)
  assert.equal(installDoc.includes("mode=test"), true)
