...
test("windows portable workflow uploads a test artifact without publishing a release", () => {
  const workflow = readFileSync(join(repoRoot, ".github/workflows/windows-portable.yml"), "utf8")
  const installDoc = readFileSync(join(repoRoot, "docs/install.md"), "utf8")
  ...
  assert.equal(workflow.includes("--bundles msi"), false)
  assert.equal(workflow.includes("*.msi"), false)
  assert.equal(installDoc.includes("windows portable test build"), true)
