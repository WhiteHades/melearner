...
test("windows msi workflow uploads a signed test artifact without publishing a release", () => {
  ...
  assert.equal(signingScript.includes("New-SelfSignedCertificate"), true)
  assert.equal(signingScript.includes("signtool.exe"), true)
  assert.equal(signingScript.includes("sign /s My /sha1"), true)
  assert.equal(signingScript.includes("verify /pa /v"), true)
  assert.equal(signingScript.includes("Export-Certificate"), true)
