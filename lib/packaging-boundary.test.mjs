...
test("windows msi workflow uploads a signed test artifact without publishing a release", () => {
  ...
  assert.equal(signingScript.includes("certutil.exe"), true)
  assert.equal(signingScript.includes("-addstore"), true)
  ...
  assert.equal(signingScript.includes("TrustedPublisher"), true)
