...
test("windows msi workflow uploads a signed test artifact without publishing a release", () => {
  ...
  assert.equal(signingScript.includes("openssl.exe"), true)
  assert.equal(signingScript.includes("pkcs12"), true)
  ...
  assert.equal(signingScript.includes('@("verify", "/pa", "/v", $msi.FullName)'), true)
  assert.equal(signingScript.includes("extendedKeyUsage = codeSigning"), true)
