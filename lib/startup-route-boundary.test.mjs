...
test("startup route is owned by Rust before frontend bootstrap", () => {
  ...
  assert.equal(rustEntrypoint.includes("startup_initialization_script"), true)
...
test("frontend bootstrap does not block hydration on startup route invoke", () => {
  ...
  assert.ok(appBootstrap.indexOf("hydrateLibrary(library.courses") < appBootstrap.indexOf("await resolveStartupRouteBeforeHydration(library.courses"))
  assert.ok(appBootstrap.indexOf("schedulePostHydrationWork(() =>") > appBootstrap.indexOf("hydrateLibrary(library.courses"))
  assert.equal(appBootstrap.includes("window.requestAnimationFrame"), false)
