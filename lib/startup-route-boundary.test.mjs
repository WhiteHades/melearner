...
test("frontend bootstrap does not block hydration on startup route invoke", () => {
  ...
  assert.equal(homeScreen.includes("pendingStartupRouteRef"), true)
  assert.equal(homeScreen.includes("scheduleAfterPaint"), false)
  assert.equal(homeScreen.includes("home.startupRoute.applyDeferred"), true)
  assert.equal(homeScreen.includes("setStartupRoute(null)"), true)
  assert.equal(homeScreen.includes("setPendingStartupRoute"), false)
