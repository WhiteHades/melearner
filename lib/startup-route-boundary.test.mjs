...
test("frontend bootstrap does not block hydration on startup route invoke", () => {
  ...
  assert.equal(appBootstrap.includes('new PopStateEvent("popstate"'), true)
  assert.equal(appBootstrap.includes("applyStartupRouteAfterHydration"), true)
  assert.equal(appBootstrap.includes("hydrateLibrary(library.courses, library.libraryPath)"), true)
  assert.ok(appBootstrap.indexOf("hydrateLibrary(library.courses") < appBootstrap.indexOf("applyStartupRouteAfterHydration(library.courses)"))
