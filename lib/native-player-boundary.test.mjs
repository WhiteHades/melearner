...
test("native player requests a native surface render after accepted media load", () => {
  ...
  assert.equal(linuxGtkSurface.includes("self.schedule_render();"), true)
  assert.equal(linuxGtkSurface.includes("state.render(&self.gl_area);"), false)
})
...
test("linux playback does not open a separate video window", () => {
  ...
  assert.equal(guard.includes("separate native video windows are disabled"), true)
  assert.equal(guard.includes("return Ok(())"), false)
  assert.equal(rustSurface.includes('"melearner video"'), false)
  assert.equal(rustSurface.includes("in-window render-api surface"), true)
  assert.ok(
    windowHandleAttach.indexOf("reject_linux_overlay_surface_by_default()?") <
      windowHandleAttach.indexOf("build_surface_window"),
    "Linux window-handle path must fail before building a surface window"
  )
  assert.ok(
    rustSurface.indexOf("fn reject_linux_overlay_surface_by_default") <
      rustSurface.indexOf("tauri::WindowBuilder::new"),
    "Linux guard must remain defined before any separate window construction"
  )
})
...
test("linux render-api surface uses gtk in-window host", () => {
  ...
  const requestRender = sourceBetween(linuxGtkSurface, "    fn request_render(&self)", "impl Drop for GtkInWindowSurface")
  ...
  assert.equal(requestRender.includes("self.schedule_render();"), true)
