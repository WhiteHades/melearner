...
test("macos native surface converts webview top-left bounds into appkit coordinates", () => {
  const macosSurface = readFileSync(join(repoRoot, "src-tauri/src/native_player/surface/macos_appkit.rs"), "utf8")

  assert.equal(macosSurface.includes("fn ns_rect_for_surface_in_view"), true)
  assert.equal(macosSurface.includes("view.isFlipped()"), true)
  assert.equal(macosSurface.includes("frame.size.height - rect.y as f64 - rect.height as f64"), true)
  assert.equal(macosSurface.includes("ns_rect_for_surface_in_view(webview, rect)"), true)
  assert.equal(macosSurface.includes("ns_rect_for_surface_in_view(&superview, rect)"), true)
})
...
test("linux native GLArea render consumes the draw signal after libmpv renders", () => {
  const linuxSurface = readFileSync(join(repoRoot, "src-tauri/src/native_player/surface/linux_gtk.rs"), "utf8")
  ...
  assert.equal(renderHandler.includes("state.borrow_mut().render(area)"), true)
  assert.equal(renderHandler.includes("gtk::glib::Propagation::Stop"), true)
  assert.equal(renderHandler.includes("gtk::glib::Propagation::Proceed"), false)
})

test("linux native GLArea placement is owned by the GTK overlay", () => {
  const linuxSurface = readFileSync(join(repoRoot, "src-tauri/src/native_player/surface/linux_gtk.rs"), "utf8")
  const applyRect = sourceBetween(linuxSurface, "fn apply_rect", "fn surface_length_to_i32")
  const zeroOriginAllocation = sourceBetween(linuxSurface, "fn zero_origin_allocation", "struct GtkRenderState")

  assert.equal(applyRect.includes("*layer_allocation.borrow_mut()"), true)
  assert.equal(applyRect.includes("overlay.add_overlay(gl_area)"), true)
  assert.equal(applyRect.includes("gl_area.size_allocate"), false)
  assert.equal(linuxSurface.includes("gl_area.size_allocate(&self.layer_allocation.borrow())"), false)
  assert.equal(linuxSurface.includes("zero_origin_allocation(&layer_allocation.borrow())"), true)
  assert.equal(zeroOriginAllocation.includes("gtk::Rectangle::new(0, 0,"), true)
})
...
test("native player logs load boundary and surface diagnostics", () => {
  ...
  assert.equal(rustEntrypoint.includes('const FRONTEND_LOG_ENV: &str = "MELEARNER_FRONTEND_LOG"'), true)
  assert.equal(rustEntrypoint.includes("fn frontend_log_path_from_values"), true)
  assert.equal(rustEntrypoint.includes("let log_path = frontend_log_path();"), true)
})
...
test("cross-platform native playback verifier checks packaged render diagnostics", () => {
  ...
  assert.equal(verifier.includes("listWindowsForProcess"), true)
  assert.equal(verifier.includes("melearner video"), true)
  assert.equal(verifier.includes("MELEARNER_EXPECT_AUDIO_TRACKS"), true)
  assert.equal(verifier.includes("MELEARNER_EXPECT_SUBTITLE_TRACKS"), true)
  assert.equal(verifier.includes("MELEARNER_EXPECT_CHAPTERS"), true)
  assert.equal(verifier.includes("MELEARNER_FRONTEND_LOG: frontendLog"), true)
  assert.equal(verifier.includes("MELEARNER_DB_PATH: dbPath"), true)
  assert.equal(verifier.includes("process.env.MELEARNER_DB_PATH"), true)
  assert.equal(verifier.includes("process.env.LOCALAPPDATA"), true)
  assert.equal(verifier.includes('"Application Support"'), true)
...
test("native player bridge exposes native surface visibility control", () => {
  ...
  assert.equal(rustPlayer.includes("pub async fn native_player_set_surface_visible"), true)
...
test("native player bounds sync does not initialize libmpv before load", () => {
  ...
  const loadCommand = sourceBetween(
    ...
    "#[tauri::command]\npub async fn native_player_state"
  )
...
test("native player state reads do not initialize libmpv before load", () => {
  ...
  const stateCommand = sourceBetween(
    rustPlayer,
    "pub async fn native_player_state",
    "#[tauri::command]\npub async fn native_player_play"
  )
  ...
  assert.equal(
    stateCommand.includes("finish_state_command"),
    false,
    "State reads should return snapshots without emitting full state events before resolving."
  )
  assert.equal(
    stateCommand.includes("emit_native_state"),
    false,
    "Readiness polling must not broadcast full metadata on every state read."
  )
})

test("native player commands dispatch blocking work away from the UI thread", () => {
  const rustPlayer = readFileSync(join(repoRoot, "src-tauri/src/native_player.rs"), "utf8")
  const dispatcher = sourceBetween(
    rustPlayer,
    "async fn dispatch_native_player_command",
    "fn emit_native_error"
  )
  const commands = [
    "native_player_load",
    "native_player_state",
    ...
    "native_player_set_muted",
    "native_player_set_rate",
    "native_player_select_audio_track",
    "native_player_select_subtitle_track",
    "native_player_select_chapter",
    "native_player_set_bounds",
    "native_player_set_surface_visible",
    "native_player_step_frame",
    "native_player_screenshot",
    "native_player_destroy",
  ]

  assert.equal(dispatcher.includes("tauri::async_runtime::spawn_blocking"), true)
  for (const command of commands) {
    assert.equal(
      rustPlayer.includes(`pub async fn ${command}`),
      true,
      `${command} must not block the platform UI thread.`
    )
  }
})
...
test("native player media commands require a loaded player", () => {
  ...
  const commandRanges = [
    ["pub async fn native_player_play", "#[tauri::command]\npub async fn native_player_pause"],
    ["pub async fn native_player_pause", "#[tauri::command]\npub async fn native_player_seek"],
    ["pub async fn native_player_seek", "#[tauri::command]\npub async fn native_player_set_volume"],
    ["pub async fn native_player_set_volume", "#[tauri::command]\npub async fn native_player_set_muted"],
    ["pub async fn native_player_set_muted", "#[tauri::command]\npub async fn native_player_set_rate"],
    ["pub async fn native_player_set_rate", "#[tauri::command]\npub async fn native_player_select_audio_track"],
    ["pub async fn native_player_select_audio_track", "#[tauri::command]\npub async fn native_player_select_subtitle_track"],
    ["pub async fn native_player_select_subtitle_track", "#[tauri::command]\npub async fn native_player_select_chapter"],
    ["pub async fn native_player_select_chapter", "#[tauri::command]\npub async fn native_player_set_bounds"],
    ["pub async fn native_player_step_frame", "#[tauri::command]\npub async fn native_player_screenshot"],
    ["pub async fn native_player_screenshot", "#[tauri::command]\npub async fn native_player_destroy"],
  ]
...
test("native player file lifecycle events come from libmpv events", () => {
  const rustPlayer = readFileSync(join(repoRoot, "src-tauri/src/native_player.rs"), "utf8")
  const loadCommand = sourceBetween(rustPlayer, "pub async fn native_player_load", "#[tauri::command]\npub async fn native_player_state")
  const positionLoop = sourceBetween(rustPlayer, "fn start_position_events", "fn start_playback_events")
  ...
  assert.equal(loadCommand.includes("emit_native_state(&app, &state)"), false)
...
test("native player does not synchronously force surface rendering during media load", () => {
  const rustPlayer = readFileSync(join(repoRoot, "src-tauri/src/native_player.rs"), "utf8")
  ...
  assert.equal(loadMethod.includes("surface.request_render()?"), false)
  assert.equal(loadMethod.includes("wait_until_surface_render_ready"), false)
  assert.equal(playerEntrypoint.includes("waitForReadyNativeRender(loadSnapshot.path, loaded)"), true)
  assert.equal(playerEntrypoint.includes("NATIVE_RENDER_READY_TIMEOUT_MS"), true)
  assert.equal(playerEntrypoint.includes("getNativePlayerState()"), true)
  assert.equal(surfaceBackend.includes("pub(super) fn request_render"), false)
  assert.equal(linuxGtkSurface.includes("pub(super) fn request_render"), false)
  assert.equal(linuxGtkSurface.includes("surface.request_render()"), false)
  assert.equal(linuxGtkSurface.includes("fn render_now(&self)"), false)
  assert.equal(macosSurface.includes("pub(super) fn request_render"), false)
  assert.equal(macosSurface.includes("fn render_now(&mut self)"), true)
  assert.equal(windowsSurface.includes("pub(super) fn request_render"), false)
  assert.equal(windowsSurface.includes("fn render_now(&mut self)"), true)
})
...
test("native player IPC state sanitizes non-finite mpv numbers", () => {
  const rustPlayer = readFileSync(join(repoRoot, "src-tauri/src/native_player.rs"), "utf8")
  const positionEvent = sourceBetween(rustPlayer, "    fn position_event(&self)", "    fn track_state(&self)")

  assert.equal(rustPlayer.includes("fn finite_or(value: f64, fallback: f64) -> f64"), true)
  assert.equal(positionEvent.includes('self.finite_property("time-pos", 0.0)'), true)
  assert.equal(positionEvent.includes('self.finite_property("duration", 0.0)'), true)
  assert.equal(positionEvent.includes('self.finite_property("volume", 100.0)'), true)
  assert.equal(positionEvent.includes('self.finite_property("speed", 1.0)'), true)
})
...
test("linux render-api surface uses gtk in-window host", () => {
  ...
  assert.equal(linuxGtkSurface.includes("gl_area.size_allocate(&layer_allocation.borrow())"), false)
  ...
  assert.equal(linuxGtkSurface.includes("fn request_render"), false)
...
test("windows render-api surface uses child hwnd wgl in-window host", () => {
  ...
  assert.equal(windowsSurface.includes("surfaces.try_borrow_mut()"), true)
  assert.equal(updateCallback.includes("std::thread::spawn"), true)
