...
test("native surface backend is isolated from player command state", () => {
  ...
  assert.equal(attachmentEnum.includes("WindowHandle { window: Window"), true)
...
test("linux overlay video surface is diagnostic-only", () => {
  ...
  const attach = sourceBetween(rustSurface, "pub(super) fn attach", "        match NativeSurfaceBackendPreference::current()?")
  ...
  assert.equal(attach.includes("reject_linux_overlay_surface_by_default()?"), true)
  ...
  assert.ok(
    rustSurface.indexOf("reject_linux_overlay_surface_by_default()?") <
      rustSurface.indexOf("tauri::WindowBuilder::new"),
    "Linux overlay guard must run before any diagnostic overlay window can be built"
  )
})
