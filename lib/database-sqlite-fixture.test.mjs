...
test("frontend database writes avoid manual sqlite transaction commands", () => {
  ...
  assert.equal(source.includes('database.execute("BEGIN'), false)
  assert.equal(source.includes('database.execute("COMMIT")'), false)
  assert.equal(source.includes('database.execute("ROLLBACK")'), false)
  assert.ok(
...
class RejectManualTransactionDatabase extends TestDatabase {
  async execute(sql, params = []) {
    const normalized = sql.trim().toUpperCase()
    if (normalized === "BEGIN" || normalized === "BEGIN IMMEDIATE" || normalized === "COMMIT" || normalized === "ROLLBACK") {
      throw new Error(`manual transaction command is not supported by this adapter: ${normalized}`)
    }
...
class SlowProgressDatabase extends TestDatabase {
  progressDelayUsed = false

  async execute(sql, params = []) {
    if (sql.trim().startsWith("UPDATE lessons SET watched_time") && !this.progressDelayUsed) {
      this.progressDelayUsed = true
...
test("syncLibrary works with a pooled adapter that rejects frontend transaction commands", async (t) => {
  const database = new RejectManualTransactionDatabase()
  __setDatabaseForTests(database)
  t.after(() => {
    __setDatabaseForTests(null)
    database.close()
  ...
  const result = await syncLibrary([testCourse({ id: "scan-course-a", path: "/library/Pooled" })], "/library")

  assert.equal(result.courses.length, 1)
  assert.equal(result.courses[0].id, "scan-course-a")
  const persisted = await loadPersistedLibrary()
  assert.equal(persisted.courses.length, 1)
  assert.equal(persisted.courses[0].sections[0].lessons[0].path, "/library/Pooled/Intro/welcome.mp4")
})
...
test("updateLessonProgress serializes concurrent SQLite writes", async (t) => {
  const database = new SlowProgressDatabase()
  __setDatabaseForTests(database)
  t.after(() => {
    __setDatabaseForTests(null)
    database.close()
