...
test("bootstrap mounts with local-url-routed home screen", () => {
  const providers = readSource(join(repoRoot, "components/app-providers.tsx"))
...
test("startup auto-scan hook is explicit and uses the normal scan path", () => {
  const homeScreen = readSource(join(repoRoot, "components/home-screen.tsx"))
  const rustEntrypoint = readSource(join(repoRoot, "src-tauri/src/lib.rs"))
  ...
  assert.equal(
    homeScreen.includes("if (!hasHydrated) return\n    const path = readAutoScanPath()"),
    true,
    "Startup auto-scan should wait until persisted-library hydration releases SQLite."
  )
  assert.equal(
    homeScreen.includes("const STARTUP_AUTO_SCAN_DELAY_MS = 10000"),
    true,
    "Startup auto-scan should allow post-hydration database reads to settle before saving a scan."
  )
  assert.equal(
    homeScreen.includes("window.setTimeout(() => {"),
    true,
    "Startup auto-scan should be delayed instead of racing the hydration effect stack."
  )
  assert.equal(
