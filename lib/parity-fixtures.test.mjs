...
const fixtureManifest = JSON.parse(
  readFileSync(join(fixtureRoot, "fixture-manifest-v1.json"), "utf8"),
)

test("fixture manifest pins every current parity input and its consumers", () => {
  assert.equal(fixtureManifest.version, 1)
  assert.deepEqual(
    fixtureManifest.fixtures.map((fixture) => fixture.id),
    [
      "current-native-database",
      "parity-oracle",
      "h264-aac-mp4",
      "h264-multi-audio-chapters",
      "hevc-main10",
      "external-subtitle-srt",
      "external-subtitle-vtt",
      "media-chapter-metadata",
      "corrupt-media",
      "scanner-contract",
      "responsiveness-watchdog-app",
    ],
  )
  ...
  for (const fixture of fixtureManifest.fixtures) {
    ...
    assert.equal(paths.has(fixture.path), false, `duplicate fixture path: ${fixture.path}`)
    ids.add(fixture.id)
    paths.add(fixture.path)

    assert.match(fixture.sha256, /^[0-9a-f]{64}$/)
    assert.equal(
      ["repository-authored", "deterministic-generated"].includes(fixture.source.kind),
      true,
    )
    assert.equal(fixture.source.license, "MIT")
    assert.equal(typeof fixture.formatVersion, "string")
    assert.ok(fixture.formatVersion.length > 0)
    assert.ok(Object.keys(fixture.expected).length > 0)
    assert.ok(fixture.tests.length > 0)

    const bytes = readFileSync(join(repoRoot, fixture.path))
    const actualHash = createHash("sha256").update(bytes).digest("hex")
    assert.equal(actualHash, fixture.sha256, `${fixture.path} changed without a manifest update`)
    for (const consumer of fixture.tests) {
      readFileSync(join(repoRoot, consumer))
    }
    if (fixture.generator !== null) {
      assert.equal(fixture.source.kind, "deterministic-generated")
      assert.equal(fixture.generator.tool, "ffmpeg")
      assert.equal(fixture.generator.version, "n8.1.2")
      const generatorBytes = readFileSync(join(repoRoot, fixture.generator.path))
      const generatorHash = createHash("sha256").update(generatorBytes).digest("hex")
      assert.equal(generatorHash, fixture.generator.sha256)
    }
  }
})
